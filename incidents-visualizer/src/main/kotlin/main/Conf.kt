...
class Conf(parser: ArgParser) {
    ...
    val region by parser
        .storing("--region", help = "which region incidents to visualize") { File(this) }
        .default("all")
        .addValidator {
            require(value == "all" || (simraRoot.listFiles()!!.toList().map { it.nameWithoutExtension }.contains(value.toString()))) {
